...
Compatible with your current files
"""
...
# =========================
# Simple SSIM (lightweight)
# =========================
import torch.nn.functional as F

def calculate_ssim(img1, img2):
    """Very simple SSIM approximation"""
    C1 = 0.01 ** 2
    C2 = 0.03 ** 2
...
# Evaluation
# =========================
def evaluate_model(model, test_loader, snr_list, channel_type, args, device):
    model.eval()
    ...
    for snr in snr_list:
        ...
        channel = Channel(channel_type=channel_type, snr=snr).to(device)
        ...
        with torch.no_grad():
            for images, _ in test_loader:
                ...
                ssim = calculate_ssim(decoded_noisy, images)
...
def main():
    ...
    fis_model.load_state_dict(torch.load(args.fis_checkpoint, map_location=device))
    ...
    baseline_results = evaluate_model(
        baseline, test_loader, args.snr_list, args.channel, args, device
    )

    print('\n=== Evaluating FIS ===')
    fis_results = evaluate_model(
        fis_model, test_loader, args.snr_list, args.channel, args, device
    )
